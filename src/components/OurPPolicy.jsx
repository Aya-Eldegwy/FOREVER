

import { assets } from "../assets/assets"

const OurPPolicy = () => {
    return (
    <div className="flex flex-col sm:flex-row justify-around py-20 gap-12 sm:gap-2 text-center text-xs sm:text-sm md:text-base">
    <div>
        <img src={assets.exchange_icon} alt="exchange icon" className="w-12 m-auto mb-5" />
        <p className="font-medium">Easy Exchange Policy</p>
        <p className="text-gray-400">We offer hassle free exchange policy</p>
    </div>
    <div>
        <img src={assets.quality_icon} alt="exchange icon" className="w-12 m-auto mb-5" />
        <p className="font-medium"> 7 Days Return Policy</p>
        <p className="text-gray-400">We provide 7 days free return policy</p>
    </div>
    <div>
        <img src={assets.support_img} alt="support icon" className="w-12 m-auto mb-5" />
        <p className="font-medium">Beat customer support</p>
        <p className="text-gray-400"> We provide 24/7 customer support</p>
    </div>
    </div>
    )
}

export default OurPPolicy
